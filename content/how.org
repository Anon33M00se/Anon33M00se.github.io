...
Org is serviceable, there are several packages that make the html
output nicer and help keep the various ancilliary files organized.

The package I use for this is

The htmlize package is there so that if I have a code block in a given file, like this:
...
path happens to start with wanting to manage a website in og-mode,
there are worse ways to begin the journey.


* Resources

The [[https://systemcrafters.net/][System Crafters]] website has a whole series of videos related to
...
The above link covers ox-publish quite extensively. There's another
guide to using ox-publish on the Org-mode website here:
