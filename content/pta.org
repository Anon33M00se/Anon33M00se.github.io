...
accounting systems. From [[https://quickbooks.intuit.com/][QuickBooks]] to [[https://www.xero.com][Xero]], and [[https://www.netsuite.com/portal/products/erp/financial-management/finance-accounting.shtml][Netsuite]] to
[[https://ledgersmb.org/][LedgerSMB]], I've at least dabbled with a lot of alternatives.
...
problem. It was solved with the invention of [[https://en.wikipedia.org/wiki/Double-entry_bookkeeping][Double-entry Bookkeeping]]
...
In what follow below, I'm going to go into a little more depth on PTA,
...
these files, chiefly [[https://ledger-cli.org/][Ledger]], [[https://hledger.org/][hledger]], and [[https://github.com/beancount/beancount][Beancount]]. But the file
...
and writes them in PTA format. The same utility also looks a CVS
statements from the bank to make sure all the corresponding deposits
and withdrawals match up. It will also automatically flag anything it
doesn't recognize, so I can review those transactions and edit them by
hand if need be (and, more often than not, edit the utility to handle
those new cases automatically in the future).
...
python helper code to automatically parse and process a lot of
...
Assets:Inventory:WIP:FloathubDevice (WIP stands for Work In
Progress). The transaction as a whole will reduce the quantity in
inventory of all the components that were used and then record the
*value* (in USD) of all those components in the WIP account.
...
all the components that went into it's manufacture. And all of our
...
[[./static/pdf/GST_form.pdf][file:./static/img/GST_form.jpg]]  [[./static/pdf/GST_automagic.pdf][file:./static/img/GST_automagic.jpg]]
...
account *Liabilities:CA:PST:Incurred* and you call yours
*Liabilities:Property:PaidPST*, then how do we make a reusable
...
somewhat (make it a *little* bit /more/ like QuickBooks), then we
...
this for a while now. If your involved in PTA or have any thoughts on
any of this, I'd be very happy to hear from you about this: pta –at–
modiot.com
